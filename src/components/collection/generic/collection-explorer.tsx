...
/**
 * Reusable collection product explorer: live search, age/category/price
 * filters, sort, a sticky desktop filter sidebar (a Sheet on mobile), a skeleton
 * loading state, a premium empty state, and Quick View. Frontend only — filters
 * the `products` passed in; structured to swap in a Supabase query later.
 */
export function CollectionExplorer({
  ...
  const results = useMemo(() => {
    ...
    return sorted;
  }, [products, query, age, category, maxPrice, sort]);
    ...
    <div className="grid gap-8 lg:grid-cols-[260px_minmax(0,1fr)]">
      ...
      <div>
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Select value={sort} onValueChange={(v) => setSort(v as SortValue)}>
              <SelectTrigger className="h-10 w-[170px] bg-paper" aria-label="Sort by">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {SORTS.map((s) => (
        ...
        {loading ? (
          ...
          <div className="mt-8 flex flex-col items-center rounded-3xl border border-dashed border-cream-300 bg-cream-50/60 px-6 py-20 text-center">
            <span className="flex size-16 items-center justify-center rounded-full bg-cream-200 text-neem-deep">
            ...
            <p className="mt-2 max-w-sm text-sm text-ink-muted">
              Try a different search, widen the price range, or clear a filter to see more.
            </p>
